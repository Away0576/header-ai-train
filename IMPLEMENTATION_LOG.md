...
## 2026-06-06 - v0.10.2 - 默认阈值调优

状态：DONE
负责人：Codex / SESA855007

变更内容：
1. 使用 NAB 正常样本 `art_daily_no_noise.csv` 和异常样本 `art_daily_jumpsup.csv` 比较 P99、P99.5、P99.9 阈值效果。
2. 将默认 `threshold.percentile` 从 `99.0` 调整为 `99.9`。
3. 将项目版本推进到 `0.10.2`。

验证结果：
1. 原始 P99 配置下，正常样本异常窗口数为 `42`，异常样本异常窗口数为 `2105`。
2. 完整复训后的 P99.9 配置下，正常样本异常窗口数降至 `14`，异常样本异常窗口数为 `2017`。
3. P99.9 将正常样本误报率从约 `1.06%` 降至约 `0.35%`。
4. 异常样本仍有约 `50.77%` 窗口超过阈值，保留了主要异常检出能力。
5. 最终采用 P99.9 作为当前默认阈值策略。
